---
name: firetracker
description: The single skill for everything Firetrack. Use when (a) installing or adopting the Firetrack CLI in a project — set up a firetracker.yml tracking plan as the source of truth for Firebase Analytics / GA4, install the firetrack binary (install script, mise, or nest), generate type-safe Swift analytics code, or wire validate/generate into CI; (b) designing, validating, syncing, or analyzing a Firebase Analytics / GA4 tracking plan; or (c) editing the Firetrack Swift package itself — its CLI, GA4 sync logic, YAML validation, SwiftSyntax generator, tests, or docs. Triggers include "install firetrack", "set up firetrack", "add analytics tracking plan", "adopt firetrack", "design a GA4 tracking plan", "sync GA4 custom dimensions", and "work on the Firetrack package".
---

# Firetrack

Firetrack turns one YAML file (`firetracker.yml`) into the source of truth for
...
code. The config file `firetracker.yml` is the default `--config` path; the CLI binary
is `firetrack`.
...
Place `firetracker.yml` at the working-directory root (the default `--config` path;
...
```bash
firetrack validate --config firetracker.yml
firetrack generate --config firetracker.yml \
...
```bash
firetrack ga4 diff --config firetracker.yml             # read-only diff
firetrack ga4 sync --config firetracker.yml --dry-run   # preview, no changes
firetrack ga4 sync --config firetracker.yml             # create missing
firetrack doctor --config firetracker.yml               # check auth/config
```
...
```yaml
- run: firetrack validate --config firetracker.yml
- run: |
    firetrack generate --config firetracker.yml \
...
custom dimensions, custom metrics, and BigQuery link intent in `firetracker.yml`.
...
2. Run `swift test`.
3. Run `swift build`.
4. Validate the tracking plan: `swift run firetrack validate --config firetracker.yml`.
5. For GA4 work, preview with `--dry-run` before syncing for real:
   - `swift run firetrack ga4 diff --config firetracker.yml`
   - `swift run firetrack ga4 sync --config firetracker.yml --dry-run`
   - `swift run firetrack ga4 sync --config firetracker.yml`
